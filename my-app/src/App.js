...
const MainCard = ({ src, handleHeartClick, choiceFavorites, heart }) => {
...
const Favorites = ({ favorites }) => {
  return (
      <ul className="favorites">
          {favorites.map((food, idx) => <FoodItem src={food} key={Date.now() + idx} />)}
      </ul>
  );
}
...
const App = () => {
  ...
  const [heart, setHeart] = React.useState(() => {
      return jsonLocalStorage.getItem('heart');
  });
  ...
  function handleHeartClick() {
      setHeart((pre) => {
          ...
          jsonLocalStorage.setItem('heart', nextHeart);
          return nextHeart;
      });
